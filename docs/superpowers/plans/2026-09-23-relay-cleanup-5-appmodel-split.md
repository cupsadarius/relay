...
**Post-implementation note (review follow-up):** the reentrant-copy behavior above changed after this task's original implementation. Rejecting a reentrant copy with `ClipboardCopyError.busy` (now removed) made a quick second Read Selection press fail with a false "No selected text found" — the second press had no way to see the first press's result. `ClipboardService.copyCurrentSelection()` now stores the in-flight copy as a `Task<String?, Error>`; a call landing while one is already running awaits that same task and returns its result instead of throwing. Only the call that actually started the copy clears the in-flight slot afterward, so a later, non-overlapping call still starts a fresh ⌘C. Cancellation stays per-caller: the underlying copy (`performCopy()`) is an unstructured task immune to any individual caller's cancellation, so it always finishes and restores the clipboard if a copy landed; `Task.checkCancellation()` runs afterward against each caller's own task, so a cancelled caller still gets `CancellationError` without disturbing another caller sharing the same in-flight copy.

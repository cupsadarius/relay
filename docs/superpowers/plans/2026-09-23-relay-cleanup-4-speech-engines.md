...
- Per the controller's explicit instruction, this branch was NOT rebased onto or merged with `main` even though `main` has moved (plan 3 merged first); the controller owns that rebase, and Step 5 (finishing-a-development-branch) was skipped for the same reason. The branch was later rebased onto `main` (now includes plan 3) by the controller.
- **Final test count after Tasks 15-21 and all review fixes for Tasks 8-14:** clean `rm -rf` of the derived-data dir, `xcodegen generate` (byte-identical `project.pbxproj`), then two full-suite runs from that same clean build: both `** TEST SUCCEEDED **`, both 1021 tests (1015 passed, 6 skipped, 0 failures).
- **Route-change manual checks, updated:** Task 13 Step 7 and Task 21 Step 3's input/output device-switch checks now exercise `AudioEngineRouteChangeDecision` first -- a spurious `AVAudioEngineConfigurationChange` post where the engine keeps running at the same format is ignored rather than failing the session (see `Relay/Audio/AudioEngineRouteChangeDecision.swift`). The checks a human still needs to run on real hardware are unchanged in substance, just re-scoped: confirm a *genuine* route change (unplugging/switching to a different physical device, not e.g. AirPods renegotiating their Bluetooth profile while the format and running state stay the same) still ends the session immediately, and that a same-format renegotiation no longer does. Real-hardware verification (including AirPods specifically) remains PENDING -- not performed by this automated session.
